...
export function MarketNode({ data, isConnectable, selected }: NodeProps<Node<EditSidebarData>>) {
  const connections = useNodeConnections({
    handleType: 'target',
    handleId: 'marketProvider_handle',
  });
  return (
    <>
      <Handle
...
export function MarketProviderNode({
  data,
  isConnectable,
  selected,
}: NodeProps<Node<EditSidebarData>>) {
...
export function UnitOperatorNode({
  data,
  isConnectable,
  selected,
}: NodeProps<Node<EditSidebarData>>) {
...
export function MarketProductNode({
  data,
  isConnectable,
  selected,
}: NodeProps<Node<EditSidebarData>>) {
  ...
  return (
    ...
    >
      <div className={`px-2 py-0.5 shadow rounded bg-white border text-[11px] ${border_color}`}>
        ...
        <div className="text-[10px] text-gray-500 truncate max-w-[120px]">{name}</div>
      </div>
    </div>
  );
}

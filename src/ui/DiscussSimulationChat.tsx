...
export default function DiscussSimulationChat({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: 'assistant',
      content: 'Ask questions about your simulation. I will use the current world JSON as context.',
    },
  ]);
        ...
        {messages.map((m, idx) => (
          <div
            ...
            className={`rounded-md px-3 py-2 border ${
              m.role === 'user' ? 'border-emerald-200 bg-emerald-50' : 'border-gray-200 bg-gray-50'
            }`}
          >
            ...
            <ReactMarkdown
              ...
              components={{
                code(props: { inline?: boolean; children?: React.ReactNode; className?: string }) {
                  ...
                  if (inline) {
                    return (
                      <code className={`bg-gray-100 rounded px-1 py-0.5 ${className ?? ''}`}>
                        {children}
                      </code>
                    );
                  }

...
class NodeConfig:
    ...
    def __getitem__(self, key: str):
        ...
        try:
            ...
            raise ValidationError(f"{key} is required", id=self.data['name'], field=key)
    ...
    def get_date(self, key: str, default=""):
        ...
        try:
            ...
            raise ValidationError(f"invalid date", id=self.data['name'], field=key)
...
class EdgeConfig:
    ...
    def __getitem__[T](self, key: str, default=None):
        try:
            return self.data[key]
        except KeyError:
...
class Config:
    def __init__(self, data: dict):
        ...
        self.index, self.start, self.end = self._simulation_time()
    ...
    def _simulation_time(self) -> tuple[pd.Timestamp, pd.Timestamp, pd.DatetimeIndex]:
        ...
        return start, end, pd.date_range(start=start, end=end + timedelta(hours=24), freq=world_cfg["frequency"]),
    ...
    def get_edges(self, unit_id: str, edge_type: EdgeType):
        try:
            ...
            raise ValueError(f"No edges of type {edge_type.value} found for unit {unit_id}")
